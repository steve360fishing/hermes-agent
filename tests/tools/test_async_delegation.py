...
def test_batch_accepts_requested_id_and_capacity_units():
    gate = threading.Event()
    ...
    dispatched = ad.dispatch_async_delegation_batch(
        ...
        capacity_units=2,
        delegation_id="deleg_live_transcript_id",
    )
    assert dispatched == {
        "status": "dispatched",
        "delegation_id": "deleg_live_transcript_id",
    ...
    record = next(
        ...
        if item["delegation_id"] == "deleg_live_transcript_id"
    )
    assert record["capacity_units"] == 2
    gate.set()
    assert _drain_for("deleg_live_transcript_id") is not None
